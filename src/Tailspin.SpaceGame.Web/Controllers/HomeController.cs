    ...
    public class HomeController : Controller
    {
        ...
        )
        {
            ...
            try
            {
                ...
                var scores = await _scoreRepository.GetItemsAsync(
                ...
                foreach (var score in scores) profiles.Add(_profileRespository.GetItemAsync(score.ProfileId));
                Task.WaitAll(profiles.ToArray());

                // Combine each score with its profile.
                vm.Scores = scores.Zip(profiles,
                    (score, profile) => new ScoreProfile {Score = score, Profile = profile.Result});
